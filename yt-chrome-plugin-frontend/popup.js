          ...
          <div class="section">
            ...
            <ul class="comment-list" style="list-style:none;padding:0;margin:0;">
              ${predictions.slice(0, 25).map((item, index) => {
                    let sentimentLabel = item.sentiment === '1' ? 'Positive' : item.sentiment === '0' ? 'Neutral' : 'Negative';
                    let sentimentColor = item.sentiment === '1' ? '#51cf66' : item.sentiment === '0' ? '#ffd43b' : '#ff6b6b';
                    return `
                  <li class="comment-item" style="margin:12px 0;padding:16px;background:#181828;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,0.08);border-left:4px solid ${sentimentColor};">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                      <span style="color:#aaa;font-size:0.95em;">#${index + 1}</span>
                      <span style="color:#aaa;font-size:0.95em;">${new Date(item.timestamp).toLocaleString()}</span>
                    </div>
                    <div style="margin-bottom:8px;color:#fff;line-height:1.5;">${item.comment}</div>
                    <div style="font-weight:600;color:${sentimentColor};font-size:1em;">${sentimentLabel}</div>
                  </li>
                `;
                }).join('')}
            </ul>
    ...
    async function getSentimentPredictions(comments) {
        try {
            ...
            if (response.ok && Array.isArray(result)) {
                return result; // Success: result is an array
